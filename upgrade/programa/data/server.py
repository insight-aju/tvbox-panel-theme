import os, json, subprocess, time, logging, hashlib, mimetypes, shlex
...
- FRONTEND (arquivos na pasta static/)
  - index.html
      - Painel principal:
          ...
          - Card "Controle de acionamentos" (relÃƒÂ©s R1..R4)
      - Usa principalmente:
          - GET  /api/vol
          - POST /api/vol/set
          - POST /api/mute
  ...
  - esp.html
      - Pagina de configuraÃƒÂ§ÃƒÂ£o / diagnÃƒÂ³stico:
...
- BACKEND (este arquivo)
  ...
  - MantÃƒÂ©m cache e logs em disco.
"""

# ============== BINÃƒÂRIOS EXTERNOS (ANDROID / TERMUX) ==============
BIN = {
...
# Flask - NÃƒÆ’O usamos static_url_path="" porque vamos interceptar (prioridade: GitHub -> cache -> local)
app = Flask(__name__)

# Guarda o ÃƒÂºltimo volume nÃƒÂ£o-zero (para toggle de mute da stream principal)
_last_nonzero = 10

# =================== AtualizaÃƒÂ§ÃƒÂ£o via GitHub (remote + fallback local) ===================
# Ideia:
# - VocÃƒÂª pode hospedar index.html / esp.html / style.css / images/* no GitHub (raw ou pages).
# - O servidor tenta pegar primeiro do remoto, salva em cache e serve para os clientes.
# - Se nÃƒÂ£o conseguir (sem internet / GitHub fora / URL errada), usa o arquivo local em /static.
...
# Problema clÃƒÂ¡ssico em internet instÃƒÂ¡vel:
# - O painel ÃƒÂ© local, mas se o servidor tentar buscar GitHub "antes" de servir o arquivo local,
#   qualquer falha de DNS/Internet faz a pÃƒÂ¡gina demorar MUITO para renderizar (especialmente CSS + background).
#
# EstratÃƒÂ©gia adotada aqui:
# 1) Se existir arquivo em cache (data/remote_cache), ele pode ser servido imediatamente.
# 2) Se nÃƒÂ£o existir cache (ou cache for mais antigo que o arquivo local), serve o arquivo local (/static).
# 3) O download remoto sÃƒÂ³ acontece em chamadas explÃƒÂ­citas de sincronizaÃƒÂ§ÃƒÂ£o (ou quando o arquivo local nÃƒÂ£o existe).
#
# AlÃƒÂ©m disso, quando ocorrer falha de rede, aplicamos um "cooldown" para nÃƒÂ£o ficar tentando de novo a cada request.
...
def _remote_assets_fail_cooldown_s() -> float:
    ...
    r = cfg.get("remote_assets", {}) if isinstance(cfg, dict) else {}
    try:
        return float(r.get("fail_cooldown_s", 90.0))  # padrÃƒÂ£o: 90s sem novas tentativas apÃƒÂ³s falhar
    except Exception:
        return 90.0

def _read_cache_bytes(rel_path: str):
    """LÃƒÂª bytes do cache (data/remote_cache) sem exigir base_url remoto."""
...
def _make_asset_response(data: bytes, rel_path: str, src: str):
    ...
    # HTML: sempre revalidar. CSS/imagens: pode cachear, mas como o servidor ÃƒÂ© local, revalidar ÃƒÂ© barato.
    ext = (os.path.splitext(rel_path)[1] or "").lower()
    if ext in (".html",):
        resp.headers["Cache-Control"] = "no-cache"
    else:
        # Permite cache, mas forÃƒÂ§a revalidaÃƒÂ§ÃƒÂ£o (evita ficar preso em versÃƒÂ£o velha apÃƒÂ³s Sync UI)
...
def _cached_remote_get_bytes(rel_path: str, force: bool = False, respect_ttl: bool = True):
    ...
    try:
        ...
        # Resposta vÃƒÂ¡lida, mas sem conteÃƒÂºdo ÃƒÂºtil (ou status != 2xx/304). Tenta cache/local.
        if not r.ok:
...
def serve_asset(rel_path: str):
    ...
    Importante: em rede instÃƒÂ¡vel, NÃƒÆ’O tentamos GitHub durante o carregamento normal da UI,
    para evitar travar a renderizaÃƒÂ§ÃƒÂ£o por falha de DNS/Internet.
    """
    # 1) cache (se existir e nÃƒÂ£o for mais antigo que o arquivo local)
    rel = _safe_relpath(rel_path)
    if not rel:
        abort(404)
    ...
    if cache_data is not None:
        # Se o arquivo local for mais novo (ex: vocÃƒÂª atualizou o programa via zip), prefere local.
        try:
    ...
    if os.path.exists(local_path):
        ...
        # Para HTML, revalidar sempre; para outros, revalidar rÃƒÂ¡pido (local) e nÃƒÂ£o prender versÃƒÂ£o velha
    ...
    # favicon ÃƒÂ© opcional: se nÃƒÂ£o existir local/cache, nÃƒÂ£o tenta baixar do GitHub (evita travar por DNS)
    if rel == "favicon.ico":
        return Response(b"", status=204)

# 3) remoto (sÃƒÂ³ se nÃƒÂ£o existir local)
...
# =================== PersistÃƒÂªncia de configuraÃƒÂ§ÃƒÂ£o ===================
def load_cfg():
    """
...
def save_cfg(cfg):
    """Salva config.json (usado apÃƒÂ³s mudanÃƒÂ§as em CFG)."""
...
def log(msg):
    """
    Registra linha de log no buffer em memÃƒÂ³ria e em arquivo logs/esp.log.

    Consumido por:
      - /api/logs                  (esp.html -> card "Logs do servidor")
      - FunÃƒÂ§ÃƒÂµes internas (status, esp-state-sink, comandos IR/GPIO etc).
    """
...
# =================== UtilitÃƒÂ¡rios Termux (volume / comandos) ===================
...
def parse_termux_volume(out: str):
    """
    Converte saÃƒÂ­da JSON do termux-volume em dict:
...
def get_current():
    ...
    raise RuntimeError("stream music/media nÃƒÂ£o encontrada")
...
# =================== CFG save throttling (reduce I/O on Android storage) ===================
# A maioria das rotas escreve em CFG por conta do "esp_state" (telemetria).
# Escrever no disco a cada request pode travar o Flask e causar timeouts no ESP (HTTPClient Code:-11).
# Esta camada limita saves para, no max, 1 a cada _CFG_SAVE["min_gap"] segundos.
_CFG_SAVE_LOCK = threading.Lock()
_CFG_SAVE = {"dirty": False, "last_ts": 0.0, "min_gap": 5.0}

def mark_cfg_dirty():
    with _CFG_SAVE_LOCK:
        _CFG_SAVE["dirty"] = True

def maybe_save_cfg(force: bool = False) -> bool:
    now = time.time()
    with _CFG_SAVE_LOCK:
        dirty = bool(_CFG_SAVE.get("dirty", False))
        last_ts = float(_CFG_SAVE.get("last_ts", 0.0) or 0.0)
        min_gap = float(_CFG_SAVE.get("min_gap", 5.0) or 5.0)
        if not force:
            if (not dirty) or ((now - last_ts) < min_gap):
                return False
        # reserva o slot de save (libera lock antes de I/O)
        _CFG_SAVE["dirty"] = False
        _CFG_SAVE["last_ts"] = now
    try:
        save_cfg(CFG)
        return True
    except Exception:
        # se falhar, marca dirty novamente
        with _CFG_SAVE_LOCK:
            _CFG_SAVE["dirty"] = True
        return False


# =================== NormalizaÃƒÂ§ÃƒÂ£o de estado do ESP ===================
"""
def normalize_state(resp):
    ...
    # mutes (toggle): prefer ESP `ir_mutes` (novo), senÃƒÂ£o `mute_state` (legado), senÃƒÂ£o CFG
...
def normalize_state(resp):
    ...
    # relÃƒÂ©s/entradas (mantÃƒÂ©m seu mapeamento atual)
    pin_map = [(23, "r1"), (22, "r2"), (21, "r3"), (19, "r4")]
    relays = {}
    for pin, name in pin_map:
        val = gpio.get(str(pin), gpio.get(pin))
    ...
    wifi_val = resp.get("wifi", None)
    if wifi_val is None:
        # alguns firmwares não enviam "wifi"; inferimos pelo SSID
        ss = resp.get("ssid")
        wifi_val = bool(ss) if isinstance(ss, str) else False
    out["wifi"] = bool(wifi_val)
...
def esp_base():
    """Retorna URL base do ESP (ex: http://192.168.0.150) ou None se nÃƒÂ£o configurado."""
...
# Estado para logs de transiÃƒÂ§ÃƒÂ£o de polling (evitar flood de logs)
POLL_HINT_SECS = 3
...
def _poll_log_ok_once():
    if not _poll_state["announced"]:
        log(
            f"Solicitando estados do ESP e sincronizando em background (Ã¢â€°Ë†{POLL_HINT_SECS}s). "
            "Novos logs sÃƒÂ³ em caso de falha ou reconexÃƒÂ£o."
        )
...
def _poll_transition(ok: bool):
    ...
    if ok:
        if prev is False:
            log("ConexÃƒÂ£o restabelecida: estado do ESP atualizado novamente.")
        elif prev is None:
            _poll_log_ok_once()
    else:
        if prev is not False:
            log("Falha ao atualizar estado do ESP (timeout/erro). Mantendo ÃƒÂºltimo estado salvo.")
...
# Locks/Meta para evitar consultas concorrentes ao ESP e expor "online/offline" de forma confiÃƒÂ¡vel
...
def _status_min_gap(is_ok: bool) -> float:
    """Define o intervalo mÃƒÂ­nimo entre consultas REAIS ao ESP.

    - Online: rÃƒÂ¡pido (min_ok).
    - Offline: "knock" controlado (base min_fail = 3s) e backoff progressivo se ficar muito tempo caÃƒÂ­do.
    """
    if is_ok:
...
def send_esp32_command(method, endpoint, params=None):
    ...
    base = esp_base()
    if not base:
        log("ESP32 IP nÃƒÂ£o configurado, comando nÃƒÂ£o enviado.")
        return False, "ESP32 IP nÃƒÂ£o configurado"
    try:
        ...
        if method == "GET":
            ...
            return False, "MÃƒÂ©todo HTTP nÃƒÂ£o suportado"
        if r.status_code // 100 != 2:
...
def esp_page():
    """
    Entrega esp.html (ConfiguraÃƒÂ§ÃƒÂµes / DiagnÃƒÂ³stico do ESP).
...
# Catch-all: se vocÃƒÂª adicionar futuramente app.js, favicon, etc., e colocar no GitHub, funciona sem mexer no server.
@app.route("/<path:filename>")
def any_static(filename):
    # evita interceptar /api/...
    if filename.startswith("api/"):
        abort(404)
    # sÃƒÂ³ serve arquivos que existam localmente ou quando remoto estiver configurado
...
def mute_toggle():
    ...
    Usado por:
      - index.html, botÃƒÂ£o #btnMute (card "Principal")
    """
    name, cur, mx = get_current()
    global _last_nonzero
    if cur > 0:
...
def youtube():
    ...
    Usado por:
      - index.html, card "Aplicativos" -> botÃƒÂ£o #btnYouTube
    """
...
# 1) mapeia as chaves para CAMINHO REAL (recomendo padronizar tudo num lugar sÃƒÂ³)
def _video_dir():
    """DiretÃƒÂ³rio local dos vÃƒÂ­deos.
    Por padrÃƒÂ£o: static/videos (relativo ao projeto).
    VocÃƒÂª pode trocar em data/config.json, chave: "video_dir".
    Ex.: "/storage/emulated/0/movies" (caso prefira armazenamento pÃƒÂºblico).
    """
    cfg = load_cfg()
...
def video_map():
    """Mapa chave->caminho completo do vÃƒÂ­deo local (usado pelos botÃƒÂµes)."""
    d = _video_dir()
    return {
...
def sync_remote_videos(force: bool = False, respect_ttl: bool = True):
    """Sincroniza vÃƒÂ­deos locais (bemvindo/video1/saudacao) a partir de um base_url no GitHub.
    Importante: NÃƒÆ’O baixa no momento do play (evita delay). Baixa apenas quando vocÃƒÂª chamar este endpoint.
    """
    ...
    for key, local_path in video_map().items():
        ...
        # Se for "forÃƒÂ§ar download", adiciona cache-buster para evitar CDN entregando versÃƒÂ£o antiga
        if force:
        ...
        # Se foi checado recentemente (TTL) e nÃƒÂ£o ÃƒÂ© force, pula
...
def sync_remote_ui(force: bool = False, respect_ttl: bool = True):
    """PrÃƒÂ©-carrega os arquivos de UI no cache (index/esp/style/bg)."""
    base, _, _ = _remote_assets_cfg()
    if not base:
...
def play_local_mp4(path: str):
    ...
    if not ok or ("nÃƒÂ£o ÃƒÂ© possÃƒÂ­vel reproduzir" in (out or "").lower()) or ("cannot play" in (out or "").lower()):
...
def _video_play_cfg():
    ...
    flags = str(v.get("intent_flags", "0x10008000")).strip()
    if not flags:
        flags = "0x10008000"
...
def _video_pre_stop():
    ...
    # 1) Sai do VLC (HOME) para reduzir chance de "resistÃƒÂªncia" do player em foreground
    ...
    # 2) Encerra players configurados (por padrÃƒÂ£o, VLC)
...
def play_local_mp4(path: str):
    """Abre um MP4 no player padrÃƒÂ£o do Android.

    IMPORTANTE:
    - NÃƒÂ£o use o caminho do symlink do Termux (ex: /data/data/.../home/storage/...), porque outros apps NÃƒÆ’O conseguem ler.
    - Resolve para o caminho real em /storage/... e monta URI file:/// corretamente.
    - Antes de abrir, faz um "pre-stop" (HOME + force-stop) para garantir inÃƒÂ­cio imediato do novo vÃƒÂ­deo.
    - Usa flags no intent para forÃƒÂ§ar reinÃƒÂ­cio/limpeza de task (evita tocar sÃƒÂ³ apÃƒÂ³s o fim do vÃƒÂ­deo anterior).
    - Se falhar, tenta fallback com termux-open (usa FileProvider/content://).
    """
    # 0) Melhor esforÃƒÂ§o: encerra player atual antes de abrir o novo vÃƒÂ­deo
    _video_pre_stop()

    # 1) Caminho real + URI file:///
    real = os.path.realpath(os.path.abspath(path))
    uri = "file:///" + real.lstrip("/")

    # 2) Intent flags (default compatÃƒÂ­vel com o server antigo)
    vcfg = _video_play_cfg()
    flags = (vcfg.get("intent_flags") or "").strip()

    # 3) Tenta abrir via intent padrÃƒÂ£o de vÃƒÂ­deo
    ...
    # 4) Fallback via termux-open, se o intent falhar ou indicar erro de reproduÃƒÂ§ÃƒÂ£o
    low = (out or "").lower()
    if (not ok) or ("nÃƒÂ£o ÃƒÂ© possÃƒÂ­vel reproduzir" in low) or ("cannot play" in low):
        # Se existir path explÃƒÂ­cito no BIN, tente primeiro (mantÃƒÂ©m compatibilidade)
...
def go_home():
    ...
    Usado por:
      - index.html, card "Aplicativos" -> botÃƒÂ£o #btnHome (Retorno)
...
# =================== API: ConfiguraÃƒÂ§ÃƒÂ£o do ESP & Logs ===================
...
def api_logs():
    """
    Retorna ÃƒÂºltimas linhas do buffer de logs (LOG_BUF).
    Usado por:
      - esp.html:
          - Card "Logs do Servidor"
              - <pre id="logs">
              - BotÃƒÂ£o #reload-logs
    """
    try:
...
def api_state_raw():
    ...
    #Opcional para diagnÃƒÂ³sticos (nÃƒÂ£o usado diretamente pelo frontend).
    """
...
def _new_sync_job(what: str, force: bool, respect_ttl: bool):
    ...
    with _SYNC_LOCK:
        ...
        if what in ("ui", "videos"):
            _SYNC_LAST["all"] = sid  # ultimo global tambÃƒÂ©m
    return sid
...
def _run_sync_job(sid: str):
    ...
    para a barra subir gradualmente (a cada arquivo/vÃƒÂ­deo concluÃƒÂ­do).
    """
    with _SYNC_LOCK:
        ...
        if what in ("videos", "all"):
            ...
            if not base:
                ...
                for key, local_path in video_items:
                    ...
                    # TTL: se checado recentemente e existe arquivo local, pula (a nÃƒÂ£o ser force)
                    if respect_ttl and (not force) and have_local and meta.get("fetched_at") and (now - float(meta["fetched_at"]) < ttl):
...
_AUTO_UI_TICK_S = 2.0  # loop leve; checagem real sÃƒÂ³ quando TTL vence
...
def _auto_ui_worker():
    while True:
        ...
        # marca jÃƒÂ¡ no comeÃƒÂ§o p/ nÃƒÂ£o martelar quando internet cair
        with _AUTO_UI_LOCK:
            if _AUTO_UI["running"]:
                continue
        ...
        try:
            ...
            for rel in _AUTO_UI_FILES:
                # respeita cache, mas como o worker sÃƒÂ³ roda quando TTL venceu, forÃƒÂ§amos a checagem remota
...
def api_status():
    ...
    Problema que este endpoint resolve:
      - A UI pode chamar /api/status em loop (vÃƒÂ¡rios clientes).
      - Quando o ESP cai, nÃƒÂ£o podemos ficar "espancando" o ESP com requisiÃƒÂ§ÃƒÂµes concorrentes.
      - A UI precisa de um indicador confiÃƒÂ¡vel de Online/Offline (nÃƒÂ£o pode ser baseado sÃƒÂ³ em "ip existe").

    Regras:
      - Cache com gap mÃƒÂ­nimo (STATUS_CACHE) e backoff em caso de falha.
      - Anti-concorrÃƒÂªncia: apenas 1 thread faz a consulta real ao ESP por vez.
      - Compat: continua retornando HTTP 200 e {ok:true, state:{...}}; o online real fica em state.online.
    """
    global CFG
    now = time.time()
    sink_age = now - float(SINK_META.get("last_ts", 0.0) or 0.0)
    sink_alive = sink_age <= float(SINK_TTL_S)
    ...
    with STATUS_LOCK:
        ...
        # Se ainda estÃƒÂ¡ dentro da janela, responde do cache
        if age < min_gap:
            base = (STATUS_CACHE.get("state") or {}).copy()
            state_out = base.copy()
            state_out["ts"] = int(now)
            state_out["online"] = bool(STATUS_CACHE.get("ok", False))
            ...
            if not state_out.get("ip"):
                state_out["ip"] = (CFG.get("esp_ip") or SINK_META.get("last_ip") or "")
            return jsonify(ok=True, state=state_out)

        # Se outra requisiÃƒÂ§ÃƒÂ£o jÃƒÂ¡ estÃƒÂ¡ consultando o ESP, nÃƒÂ£o duplica: devolve cache (mesmo que "velho")
        ...
        # Marca que esta requisiÃƒÂ§ÃƒÂ£o farÃƒÂ¡ a consulta real
        # Se estamos recebendo push do ESP (sink), nao faz sentido puxar /state a cada segundo.
        # Isso reduz carga e evita que um pull lento bloqueie o sink (Code:-11 no ESP).
        if sink_alive and STATUS_CACHE.get("state"):
            STATUS_CACHE["ts"] = now
            STATUS_CACHE["ok"] = True
            _STATUS_META["fail_streak"] = 0
            _STATUS_META["last_ok_ts"] = now
            st = dict(STATUS_CACHE.get("state") or {})
            st["online"] = True
            st["via"] = "sink"
            if not st.get("ip"):
                st["ip"] = (CFG.get("esp_ip") or SINK_META.get("last_ip") or "")
    ...
    if ok and isinstance(res, dict):
        ...
        CFG["esp_state"] = state
        mark_cfg_dirty()
        maybe_save_cfg(force=False)
    else:
        # Fallback para ultimo estado salvo em disco
    ...
    with STATUS_LOCK:
        ...
        # meta para o cliente (nÃƒÂ£o persistimos no CFG para nÃƒÂ£o poluir)
        min_gap = _status_min_gap(bool(STATUS_CACHE.get("ok", False)))
        age = now - STATUS_CACHE["ts"]
...
_sink_keepalive = 60.0  # logar 1x/min mesmo sem mudanÃƒÂ§a


# Meta do push (ESP -> Flask). Usado para:
# - marcar "online" mesmo sem puxar /state
# - auto-cadastrar esp_ip quando vier o primeiro push
SINK_META = {"last_ts": 0.0, "last_ip": "", "last_summary_hash": None}
SINK_TTL_S = 8.0  # considera online se recebeu push nos ÃƒÂºltimos N segundos
...
def esp_state_sink():
    """
    Endpoint para o ESP dar push do estado (evita depender sÃƒÂ³ de pull /state).
    - MantÃƒÂ©m cache em memÃƒÂ³ria (STATUS_CACHE) para responder /api/status rÃƒÂ¡pido.
    - Atualiza CFG["esp_state"] (persistÃƒÂªncia throttled para nÃƒÂ£o travar o Flask).
    - Auto-cadastra CFG["esp_ip"] no 1Ã‚Âº push (request.remote_addr) quando estiver vazio.
    """
    try:
        ...
        # pode vir como {"state": {...}} ou direto {...}
        inner = state_data.get("state") if (isinstance(state_data, dict) and "state" in state_data) else state_data
        norm = normalize_state(inner)

        now = time.time()
        remote_ip = (request.headers.get("X-Forwarded-For") or request.remote_addr or "").split(",")[0].strip()

        # garante que o "ip" apareça no /api/status quando o estado vier via sink
        if remote_ip:
            norm["ip"] = remote_ip

        global _last_sink_log_ts

        # meta do sink
        SINK_META["last_ts"] = now
        SINK_META["last_ip"] = remote_ip

        # auto-cadastro / correção do IP do Controlador via sink (DHCP pode mudar)
        global CFG
        cfg_changed = False
        if remote_ip:
            old_ip = str(CFG.get("esp_ip", "")).strip()
            if old_ip != remote_ip:
                CFG["esp_ip"] = remote_ip
                cfg_changed = True
                log(f"[ESP->SRV] esp_ip atualizado via sink: {old_ip or '(vazio)'} -> {remote_ip}")

        # atualiza cache em memÃƒÂ³ria imediatamente (isso ÃƒÂ© o que evita timeouts no ESP)
        with STATUS_LOCK:
            STATUS_CACHE["ts"] = now
            STATUS_CACHE["ok"] = True
        ...
        # persiste ÃƒÂºltimo estado conhecido (throttled)
        CFG["esp_state"] = norm
        mark_cfg_dirty()
        if cfg_changed:
            maybe_save_cfg(force=True)
        else:
            maybe_save_cfg(force=False)

        # log: apenas em mudanÃƒÂ§as relevantes (relay/vol/mute) ou keepalive 1x/min
        summary = {
            "gpio_states": norm.get("gpio_states") or {},
            "ir_volumes": norm.get("ir_volumes") or {},
            "mute_state": norm.get("mute_state"),
            "ir_mutes": norm.get("ir_mutes") or {},
        }
        h = _json_hash(summary)
        do_log = False
        if SINK_META.get("last_summary_hash") != h:
            do_log = True
        elif (now - float(_last_sink_log_ts or 0.0)) >= float(_sink_keepalive):
            do_log = True

        if do_log:
            SINK_META["last_summary_hash"] = h
            _last_sink_log_ts = now
            log(f"[ESP->SRV] sink ok from={remote_ip or 'unknown'}")

        return jsonify(ok=True, received=True), 200

    except Exception as e:
        log(f"[ESP->SRV] sink error: {e}")
...
def ir_command():
    ...
    if not device or not command:
        return jsonify(ok=False, error="ParÃƒÂ¢metros IR ausentes"), 400
...
def gpio_command():
    ...
    if pin is None or state is None:
        return jsonify(ok=False, error="ParÃƒÂ¢metros GPIO ausentes"), 400
...
def status_esp32():
    ...
    NÃƒÂ£o ÃƒÂ© usado diretamente por index.html/esp.html no momento.
    """
...
def wifi_config_esp32():
    ...
    Atualmente reservado para uso futuro (nÃƒÂ£o integrado no HTML).
    """
    j = request.get_json(force=True, silent=True) or {}
    ssid = j.get("ssid")
...
def start_show():
    ...
    """
    j = request.get_json(silent=True) or {}
    delay = int(j.get("delay_s", 45))  # padrÃƒÂ£o: 45s

    # 1) Liga relÃƒÂ©s R1/R2 (Painel de LED / Sistema de som)
    try:
        proxy_get("gpio", params={"pin": 23, "state": "ON"})  # r1
        proxy_get("gpio", params={"pin": 22, "state": "ON"})  # r2
        log("start_show: relÃƒÂ©s R1/R2 ligados.")
    except Exception as e:
        log(f"start_show: erro ao ligar relÃƒÂ©s -> {e}")
        # se falhar, o front ainda pode ligar manualmente; seguimos

    # 2) Executa o vÃƒÂ­deo de boas-vindas
    cmd_video = (
        'am start -a android.intent.action.VIEW '
        '-d file:///storage/emulated/0/movies/bemvindo.mp4 -t video/mp4'
    )
    ok_video, out_video = run(cmd_video, use_shell=True)
    if not ok_video:
        log(f"start_show: erro ao executar vÃƒÂ­deo bemvindo -> {out_video}")
    else:
        log("start_show: vÃƒÂ­deo bemvindo.mp4 disparado com sucesso.")

    # 3) Agenda abrir YouTube em background (nÃƒÂ£o bloqueia o Flask)
    cmd_timer = (
        f'sh -c "sleep {delay}; '
...
def stop_show():
    ...
    """
    j = request.get_json(silent=True) or {}
    delay_s = float(j.get("delay_s", 3.0))  # padrÃƒÂ£o: 3s
